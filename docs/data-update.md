...
|---|---|---|
| [poe2db.tw](https://poe2db.tw) Chronicles pages (`/tw` + `/cn` + `/us`) | `new ModsView({...})` embedded in public HTML, plus harvest-tag chips | Nothing that needs a cookie, account, or private API |
| [RePoE-fork PoE2](https://repoe-fork.github.io/poe2/) | `mods.min.json` **match-text fallback only**, and the index title (`PoE2 version x.y.z`) as a label | We do not rebuild the UI catalog from RePoE. poe2db family grouping stays canonical; Traditional, Simplified, and English effect text come from `/tw`, `/cn`, `/us` |
...
Shop/campaign defaults in `packages/data/src/campaign.ts` are **not** regenerated here. They reuse verified `match` / `matchZh` / `matchZhHans` strings and stay hand-maintained until a real chapter map model exists.
...
- **Failure modes:** empty pools, duplicate ids, missing `matchZh` / effect text, or a ≥30% family-count drop vs the previous snapshot abort the write (`exit 3`). Missing `matchZhHans` is a warning-level gap (CN page skipped) and does not abort, so a blocked `/cn` fetch cannot freeze 繁中+EN updates. Network/timeouts after retries abort without writing (`exit 2`).
...
- Campaign shop mods and chapter map nodes are out of this pipeline (`matchZhHans` on shop mods is copied from live `/cn` pages by hand).
- GitHub Actions may not be able to scrape poe2db.tw; local refresh remains the reliable live path.
- `/cn` effect grammar is **not** a 繁→简 conversion (e.g. 生命上限 vs 最大生命, 冰霜抗性 vs 冰冷抗性, 引路石 vs 換界石). Never invent Simplified affix text.
