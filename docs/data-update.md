...
| [RePoE-fork PoE2](https://repoe-fork.github.io/poe2/) | `mods.min.json` **match-text fallback only**, and the index title (`PoE2 version x.y.z`) as a label | We do not rebuild the UI catalog from RePoE. poe2db family grouping + Traditional Chinese effect text stay canonical |
...
- **Failure modes:** empty pools, duplicate ids, missing `matchZh` / effect text, or a ≥30% family-count drop vs the previous snapshot abort the write (`exit 3`). Network/timeouts after retries abort without writing (`exit 2`).
...
- Campaign shop mods and chapter map nodes are out of this pipeline.
