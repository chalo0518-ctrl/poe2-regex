...
/** Drop leftover +/- punctuation so `火焰抗性 +` becomes `火焰抗性`. */
...
export function mergeHarvestTags(twTags = [], usTags = []) {
  const byId = new Map();
...
export async function scrapeModifiersPage(path, options = {}) {
  ...
  return {
    ...
    tags: mergeHarvestTags(extractHarvestTags(twHtml), extractHarvestTags(usHtml)),
  };
}
